...
def _normalise_page_insights(page_insights: List[Dict[str, Any]] | None, page_count: int, page_cards: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for raw in page_insights or []:
        ...
        by_page[page] = {
            ...
            "page_title": _squash(raw.get("page_title", ""))[:180],
            "must_have_points": [_squash(item)[:180] for item in raw.get("must_have_points", []) if _squash(item)][:8],
...
def _must_have_points(
    ...
    for suggestion in suggestions:
        ...
        selected.append({
            ...
            "label": anchor.get("outline_label") or suggestion.get("label") or anchor.get("title"),
    ...
    if len(selected) >= 3:
        ...
        return _ensure_evidence_rich_coverage(enriched[:8], content_units)[:8]
    return _fallback_must_have_points(content_units, page_insights, profile.get("document_regime", "sectioned_document"))
...
def _document_insights(
    ...
    prompt = _document_insights_prompt(document_id, source_file, profile, primary_subject, document_language, page_insights, must_have_points, content_units, section_map)
    parsed = _llm_document_insights(prompt)
    if not parsed:
        return fallback
    suggested_outline = [item for item in parsed.get("suggested_outline", []) if item and not _is_noise_text(item)]
    ...
    suggested_title = parsed.get("suggested_title") or fallback["suggested_title"]
...
def _ensure_outline_covers_must_have(outline: List[str], must_have_points: List[Dict[str, Any]]) -> List[str]:
    ...
    for point in must_have_points:
        label = _normalize_outline_candidate(point.get("label", ""))
        if not label:
            continue
...
def _document_insights_prompt(
    ...
    return (
        ...
        "Rules:\n"
        f"- SAME language as the source document. Expected language code: {document_language}.\n"
...
def _must_have_label(unit: Dict[str, Any], regime: str) -> str:
    ...
    return unit.get("outline_label") or unit.get("title") or unit.get("summary", "")[:120]
