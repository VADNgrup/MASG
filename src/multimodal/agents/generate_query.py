...
"""
        ...
        _invoke = lambda msgs: chat(model=self.model, messages=msgs, temperature=0.4, max_tokens=16000)
        response = _invoke([
            {"role": "system", "content": system_prompt},
            {"role": "user", "content": user_prompt}
        ])

        need_visualization = self._parse_json_response(response, _invoke)
    ...
    def _parse_json_response(self, response_content: str, invoke_fn=None) -> List[Dict]:
        if invoke_fn is None:
            invoke_fn = lambda msgs: chat(model=self.model, messages=msgs, temperature=0.4, max_tokens=16000)
        return parse_json_response(response_content, invoke_fn)
