...
def _iter_entries(log_path: Path, tail: int | None = None) -> Iterator[dict]:
    """Yield parsed log entries from a JSONL file."""
...
def analyze(
    ...
    for model, b in sorted(per_model.items(), key=lambda kv: -kv[1]["total_tokens"]):
        ...
        print(COL.format(
            model[:40],
            b["calls"],
            _fmt(b["prompt_tokens"]),
            _fmt(b["completion_tokens"]),
            _fmt(b["total_tokens"]),
            f"{avg_lat:.2f}s",
        ))
        for k in totals:
    ...
    print(COL.format(
        "TOTAL",
        totals["calls"],
        _fmt(totals["prompt_tokens"]),
        _fmt(totals["completion_tokens"]),
        _fmt(totals["total_tokens"]),
        f"{avg_lat_total:.2f}s",
    ))
    print(sep)
...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        help="Path to the JSONL log file (default: logs/llm_calls.jsonl)",
    )
    p.add_argument(
        "--tail",
        type=int,
        default=None,
        metavar="N",
        help="Only analyse the last N entries.",
    )
    p.add_argument(
        "--since",
        default=None,
        metavar="DATE",
        help="Only include entries on or after this ISO date/datetime (e.g. 2026-04-05).",
    )
    return p.parse_args()
...
def plot(
    ...
    """
    Vẽ 4 biểu đồ phân tích LLM log:
      1. Số lần gọi theo model (bar)
      2. Token usage theo model (stacked bar)
      3. Latency trung bình theo model (bar)
      4. Token usage theo thời gian (line, cumulative)

    Ví dụ:
        from src.utils.llm_analysis import plot
        plot()
        plot(tail=200, since=datetime(2026, 4, 5, tzinfo=timezone.utc))
    """
    ...
    short = [m.split("/")[-1][:24] for m in models]   # tên ngắn cho label
    x = range(len(models))
    ...
    ax.set_ylabel("# calls")
    ax.set_xticks(list(x)); ax.set_xticklabels(short, rotation=25, ha="right")
    ax.grid(axis="y", alpha=0.3)

    ax = axes[0, 1]
    b1 = ax.bar(x, prompt, label="Prompt",     color=_COLORS[0], alpha=0.85)
    b2 = ax.bar(x, compl,  bottom=prompt,
                label="Completion", color=_COLORS[1], alpha=0.85)
